...
void extractFaces(const string videoLocation, const string outputLocation, const string haarCascadeXML) {
    ...
    const string FOLDER_ALL_FACES = "AllFaces";
    ...
    string folderAllFaces = outputLocation + "/" + FOLDER_ALL_FACES;
    ...
    if( create_directory(folderAllFaces.c_str()) ) {
        cout << "Successfully created AllFaces directory" << endl;
    }
    else {
        cout << "Failed to create the folder Allfaces" << endl;
    }
    ...
    for(;;) {
        ...
        for(int i=0; i<facesInTheFrame.size(); i++) {
            ...
            string tempFaceFileName = folderAllFaces + "/" + boost::lexical_cast<string>(frameNumber) + "_" + boost::lexical_cast<string>(i) + ".jpg";
