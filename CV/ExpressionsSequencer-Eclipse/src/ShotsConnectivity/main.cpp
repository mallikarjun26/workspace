	...
	if(edgeWeightsPtr.is_open()) {
		if(getline(edgeWeightsPtr,line)) {
			...
			if(frameShotMap[frame1] < frameShotMap[frame2]) {
				...
				shot1 = frameShotMap[frame2];
				shot2 = frameShotMap[frame1];
			}

			// Mapping Shot of Shot info. First level shot number is less than or equal to second level shot number
			if(shot1 == shot2) { // Frames from same shot
				if(shotsNetwork.find(shot1) != shotsNetwork.end()){ // outer map is present
					if(shotsNetwork[shot1].find(shot1) != shotsNetwork[shot1].end() ) { // Inner map also present
						map<int, Shot> tempShotMap = shotsNetwork[shot1];
						Shot tempShot = tempShotMap[node1];
						...
						tempShot.avg_dist = edgeWeight;
						tempShot.max_dist = edgeWeight;
						shotsNetwork[shot1] = tempShotMap;
					}
				}
			...
			else { // Frames from different shot
				if(shotsNetwork.find(shot1) != shotsNetwork.end()){ // outer map is present
					if(shotsNetwork[shot1].find(shot2) != shotsNetwork[shot1].end() ) { // Inner map also present
						map<int, Shot> tempShotMap = shotsNetwork[shot1];
						Shot tempShot = tempShotMap[node2];
						if(tempShot.min_dist > edgeWeight) // min value
							tempShot.min_dist = edgeWeight;
						if(tempShot.max_dist < edgeWeight) // min value
							tempShot.max_dist = edgeWeight;
