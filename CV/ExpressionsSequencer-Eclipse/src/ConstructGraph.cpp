...
void traversalGreedyNextHop(vector< list < pair<int, float> > > &adjacencyList, list<int> &traverseList, int currentVertex, int endVertex, int noOfVerticesLeft) {
...
void constructGraph(const string outputLocation, int traversalType, int featuresType, int currentVertex=2 ){
    ...
    if(edgeWeightPtr.is_open()) {
        ...
        while(getline(edgeWeightPtr, line)) {
			...
			if (!((edgeWeight > 18) && (traversalType == 1))) {
				// Fill in the adjacency list
				//cout << "Debug 11:: Maybe here ::" << vertexA << " " << vertexB << " " << edgeWeight << " adjacencyList size:" << adjacencyList.size() << endl;
				adjacencyList[vertexA].push_back(
						make_pair(vertexB, edgeWeight));
				//cout << "Debug 22:: Maybe here" << endl;
				adjacencyList[vertexB].push_back(
						make_pair(vertexA, edgeWeight));
			}
        }
    ...
    cout << "Entered constructGraph: Traverse the graph" << endl;
    // Traverse the graph
    list<int> traverseList;
    int endVertex = 80, nextVertex;

    switch(traversalType){
    	case 0 : traversalGreedyNextHop(adjacencyList, traverseList, currentVertex, endVertex, noOfVerticesLeft); break;
    	case 1 : traversalDijkstra(noOfVertices, currentVertex, endVertex, adjacencyList, traverseList, outputLocation) ; break;
    	default: traversalGreedyNextHop(adjacencyList, traverseList, currentVertex, endVertex, noOfVerticesLeft); break;
    }
...
*/
    ...
    for(list<int>::iterator it=traverseList.begin(); it!=traverseList.end(); it++) {
        ...
        string strTemp = outputLocation + "/AllFaces/Final Set/" +  faceMap[faceNumber];
